...
BASEDIRECTORY='/var/monitor/monitor_authLog_ReactInvalidUserSSH/'
BASENAME='monitor_authLog_ReactInvalidUserSSH_'
...
## I.  Read /var/log/auth.lg and look for invalid users and store their IPs in a file.
touch ${DIRNAME}/ip.txt
tail -n 500 /var/log/auth.log | grep -E ".*ssh.*invalid user.*" | \
